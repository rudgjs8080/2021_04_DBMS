...
-- Oracle 에서는 SYSDBA 권한 등급이 있으며
-- 일반적인 DBA보다 높은 권한을 가지고 있다
...
-- 자칫 명령을 잘못입력하고 실행하면 ORACLE DBMS 자체에 문제가 생긴다
...
-- DBMS 에서 사용자 계정
-- DBMS는 사용자에게 권한 등급을 부여하여 
-- 데이터생성, 조회, 삭제, 변경 등을 세부적으로 지정할 수 있다
-- 세부적인 항목은 조직마다 별도로 정책을 만들어 사용하기 때문에
-- 규칙이 정해진것은 없다
-- 보통 DBMS에서 새로운 사용자계정을 등록하면
-- 일반적인 명령을 수행할 수 있도록 허용하는데
-- ORACLE은 새로운 사용자계정을 생성하면 아무것도 할 수 없는 상태이다
-- 관리자(SYSDBA)는 새로운 계정이 무언가 명령을 수행할 수 있도록
-- 권한을 부여하는 명령을 수행해 줘야 한다
-- 오라클은 새로 생성된 사용자 계정은 
-- DBMS에 접속하는 것 조차 막아 놓는다
