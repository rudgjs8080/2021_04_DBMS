...
-- 코드에 세미콜론을 빼먹으면 멍령문 문법오류가 발생한다

-- 1. 데이터 저장소를 생성하기
-- oracle에서는 데이터 저장소를 파일 단위로 관리한다
-- oracle에서는 데이터 저장소를 "Table Space"라고 한다
-- oracle 이외의 다른 DBMS 소프트웨어에서는 
-- 데이터 저장소를 "DataBase" 라고 한다
-- 생성하기 : Create
-- 로컬디스크의 oraclexe/data 폴더에 mydb.dbf 라는 파일을 생성하고
-- mydb라는 이름으로 사용하겠다
-- 데이터가 있던 없던 간에 기본 크기를 10MByte로 설정해 달라
-- 만약 데이터가 추가되는 동안에 공간이 부족하면
-- 자동으로 10KByte 씩 증가하여 준비해달라
...
-- SQL 코드를 입력하고 명령줄 내에서 Ctrl + Enter 를 입력
-- 가. SQL Developer 는 localhost:1521 에서 기다리고 있는
--      Oracle DBMS 서버에게 명령을 전달한다
-- 나. Oracle DBMS 서버는 명령을 수신한 후 문법검사를 하고
--      명령을 실행한 후 결과를 다시 SQL Developer에게 return한다
...
-- 사용자 생성하기
-- oracle DBMS 에 접속하여 명령을 수행할 수 있는 사용자 계정 추가
...
-- Oracle애서의 사용자 계정은 데이터 저장소와 연결하는
--      독특한 역할도 수행한다
-- A라는 사용자가 생성한, 연결된 TableSpace가 있다면
--      다른 사용자는 TableSpace에 데이터를 추가, 수정, 삭제, 조회
--      하는데 제한을 둘 수 있다
-- 사용자를 등록할 때 사용자만의 TableSpace를 지정하여 줄수 있다
--      사용자만의 데이터를 별도로 관리할 수 있다
-- 일반적인 DBMS는 데이터 저장공간을 DataBase 라는 것 만으로
--      지정을 한다.
--      만약 데이터를 추가하고자 할 때
