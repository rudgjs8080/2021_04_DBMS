...
from tbl_score;
/*
tbl_score 테이블은 학번, 국어, 영어, 수학 칼러믕로 구성되어 있다
...
어쩔수 없이 테이블을 변경해야 한다
이러한 테이블을 설계 단계부터 잘못된 설계이다
*/

-- 학생점수를 저장할 테이블을 정규화 하기 위해
-- 재설계를 하였다
-- 칼럼으로 되어 있던 과목을 데이터화 하여
-- 학번, 과목 형식으로 입력하도록 설계했다
-- 이렇게 설계를 하고 나니
-- 학번, 과목명 어떤 칼럼도 단독으로 PK로 설정할 수 없게 되었다
-- 테이블에 PK가 없으면
-- 데이터의 무결성 유지가 매우 힘든 상황이 될 수 있다
-- 이 테이블은 학번 + 과목명을 묶어서 PK로 선언을 할 수밖에 없게 된다
-- PK는 단독 칼럼으로 사용하는 것이 가장 좋은데
-- 단독 칼럼으로 만들수 없는 경우는 2개 이상의 칼럼을 묶어서
-- 만들어야 하는 경우도 있다
-- 이런 상황에 직면하면 실제 데이터에는 없는 칼럼을 하나 추가하고
-- 별도로 PK로 설정한다.
...
-- 2개 이상의 칼럼을 묶어서 PK로 선언하는 경우
-- 삭제나, 갱신을 할 때 다음과 같은 코드를 사용해야 하는데
...
-- 정규화된 테이블에서 학생별 총점과 평균 계산
select sc_num, sum(sc_score), round(avg(sc_score),1)
from tbl_score_v2
group by sc_num
order by sc_num;
-- 전체학생의 과목별 총점
select sc_subject, sum(sc_score), round(avg(sc_score),1)
from tbl_score_v2
group by sc_subject
order by sc_subject;
